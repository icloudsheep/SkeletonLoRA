...
'''
@File    :   aggregate.py
@Time    :   2024/12/05 16:09:29
@Author  :   Jianmin Liu 
@Version :   1.0
@Site    :   https://jianmin.cc
@Desc    :   Rewrite the underlying aggregation functions to implement AVG after AB multiplication.
'''
...
def aggregate_flora(results: list[tuple[ClientProxy, FitRes]],max_rank) -> NDArrays:
    """
    Use the FLORA aggregation method to calculate the weighted average of the matrix product of B_i A_i for each client,
    and rearrange the results into the original [A1, B1, A2, B2, ...] format.
    """
    import time
    ...
    max_rank = max(max_rank,max(client.properties['lora_rank'] for client,_ in results))
    ...
    # Initialize storage for the weighted results of each client
    aggregated_results = None
    SEQ_CLS = False
    BERT_TYPE =False
    for _, (client_params, scaling_factor) in enumerate(zip(parameters, scaling_factors)):
        client_result = []
        if len(client_params)%2 ==0 and client_params[-1].ndim==1:
            # Bert SeqClassficiation init NEW: ['classifier.bias', 'classifier.weight']  the last para is bias.
            num_lora_layers = len(client_params) -2 
            SEQ_CLS = True
            BERT_TYPE = True
            # print("SEQ_CLS",SEQ_CLS, " BERT_TYPE",BERT_TYPE)
        elif len(client_params)%2 ==0:
            # NORMAL NLG task
            num_lora_layers = len(client_params)
        else:
            # Llama SeqClassficiation init NEW: [score.weight]
            num_lora_layers = len(client_params) - 1  
            SEQ_CLS = True
            
        for j in range(0, num_lora_layers, 2):
            A = client_params[j]  # A_j
            B = client_params[j + 1]  # B_j
            BA = B @ A
            client_result.append(BA * scaling_factor)
        if SEQ_CLS and BERT_TYPE:
            client_result.append(client_params[-2]*scaling_factor)
            client_result.append(client_params[-1]*scaling_factor)
        elif SEQ_CLS:
            # Process the classification head of the model
            client_result.append(client_params[-1]*scaling_factor)
        # Aggregate the weighted results of each client
        if aggregated_results is None:
            aggregated_results = client_result
        else:
            aggregated_results = [
                agg + res for agg, res in zip(aggregated_results, client_result)
            ]

    end_time = time.time()
    log(
            INFO,
            f"Plaintext flora aggregation time: {end_time-start_time} seconds"
        )
    final_result = []
    temp = None
    if SEQ_CLS and BERT_TYPE:
        temp = aggregated_results[-2:]
        aggregated_results = aggregated_results[:-2]
    elif SEQ_CLS:
        temp = aggregated_results[-1]
        aggregated_results = aggregated_results[:-1]
    for aggregated_matrix in aggregated_results:
        # Perform SVD decomposition on each aggregated matrix
        U, s, Vh = torch.svd_lowrank(torch.from_numpy(aggregated_matrix),q=max_rank)
        B_aggregated = U@torch.diag(s)
        A_aggregated = Vh.T 
        # U, s, Vh = np.linalg.svd(aggregated_matrix, full_matrices=False)
        final_result.extend([A_aggregated, B_aggregated])
    if SEQ_CLS and BERT_TYPE:
        final_result.extend([temp[-2],temp[-1]])
    elif SEQ_CLS:
        final_result.extend([temp])
    end_time2 = time.time()
    log(
            INFO,
            f"Plaintext SVD time: {end_time2-end_time} seconds"
        )
    return final_result
