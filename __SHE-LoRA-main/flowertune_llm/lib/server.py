...
# from .man_typing import FitIns,FitRes  # 
...
class Server:
    ...
    def fit(self, num_rounds: int, timeout: Optional[float]) -> tuple[History, float]:
        ...
        # self.parameters = Parameters(tensor_type="", tensors=[])
        # print(self.client_manager().all())
        ...
        for current_round in range(1, num_rounds + 1):
            ...
            log(INFO, "fit round finished.")
            if res_fit[0] is NEGOTIATION:
                self.parameters = Parameters(tensor_type="", tensors=[])

            elif res_fit is not None:
    ...
    def fit_round(
        ...
        if parameters_aggregated is NEGOTIATION:
            log(
            INFO,
            "server negotiation finished, training starts at next round...",
        )
            return NEGOTIATION,metrics_aggregated

        log_dict = {}
        log_dict["FL-Round"] = server_round
        for (client,res) in results:
            log_dict[f"Client-{client.cid}-loss"] = res.metrics["train_loss"]
            log_dict[f"Client-{client.cid}-he_enc_time"] = res.metrics["he_enc_time"]
            log_dict[f"Client-{client.cid}-run_time"] = res.metrics["run_time"]
        wandb.log(log_dict )
...
def fit_client(
    ...
    end_time = timeit.default_timer()
    log(INFO,
        f"Clint {client.cid} Costs {end_time-start_time}s.")
